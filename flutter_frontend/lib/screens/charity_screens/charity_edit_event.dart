...
class _CharityEditEventPageState extends State<CharityEditEventPage> {
  ...
  Future<void> _fetchDetail() async {
    ...
    try {
      final uri = Uri.parse(ApiPath.charityEventDetail(widget.eventId));
      ...
      _selectEventType = (data['eventType'] ?? '') == '' ? null : data['eventType'];
  ...
  Future<void> _submit() async {
    final name = _nameController.text.trim(); // 後端目前用 name 做唯一識別，實際送出以 _originalName 為準
    ...
    try {
      final uriData = Uri.parse(ApiPath.editCharityEvent);
